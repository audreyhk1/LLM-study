...
def find_probabilities_per_llm(llm_name, llm_ans, llm_concordant, large_file):
    ...
    llm_probabilities = pd.DataFrame(0, index=["Correct", "Total", llm_name], columns=range(11))
    ...
    for i in range(NQUESTIONS):
        ...
        llm_probabilities.iat[1, ind] += 1

    llm_probabilities.loc[llm_name] = llm_probabilities.loc["Correct"].div(llm_probabilities.loc["Total"], axis=0).fillna(0).astype('float64')
    return llm_probabilities.loc[llm_name]
